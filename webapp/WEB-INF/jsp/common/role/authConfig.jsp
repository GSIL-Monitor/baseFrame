...
function resourceInit() {
	...
	<c:forEach items="${resourceList}" var="resource">
		zNodes.push({id:'${resource.id}', pId:'${resource.pid}', name:'${resource.name}', open:true/* ,chkDisabled:DisableMenu('${resource.pid} ')*/});
	</c:forEach>
	
	resourceTree = $.fn.zTree.init($("#resourceTree"), setting, zNodes);
}

function DisableMenu(pId) {
	var flag = false;
	if(pId == 0) flag = true
	return flag;
}
