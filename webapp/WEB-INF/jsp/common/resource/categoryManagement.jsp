...
function resourceInit() {	
	...
	<c:forEach items="${resourceList}" var="resource">
		zNodes.push({id:'${resource.id}', pId:'${resource.parentid}', name:'${resource.name}', open:true});
	</c:forEach> 
...
function addHoverDom(treeId, treeNode) {
	...
	if (btn) btn.bind("click", function(){
		...
		console.log(ids);
		openWin('添加资源' + ' - [' + treeNode.name + ']', 'common/resource/' + treeNode.id + '/${cid}/toAddChild/' + ids, 410, 358);
	});
};
...
function beforeEditName(treeId, treeNode) {
	...
	var ids = getPath(treeNode);
	openWin('编辑资源' + ' - [' + treeNode.name + ']', 'common/resource/' + treeNode.id + '/${cid}/toUpdateChild/' + ids, 410, 358);
...
function beforeRemove(treeId, treeNode) {
	...
	layer.confirm(msg, {icon: 3}, function(index){
	    ...
	    $.ajax({
			type: "GET",
			url: "common/resource/" + treeNode.id + "/${cid}/delete",
			async: false,
			success: function(data) {
				if ("Success" == data) {
					resourceTree.removeNode(treeNode);
				}else if("Failure" == data){
					layer.alert('此资源已关联文件，不允许删除', {
