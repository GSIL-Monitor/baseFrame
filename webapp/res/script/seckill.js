...
var seckillJS = {
    ...
    detail : {
        ...
        init : function(param) {
            ...
            if(!seckillJS.validatePhone(userPhone)) {
                ...
                $('#killPhoneBtn').click(function() {
                    ...
                    if(seckillJS.validatePhone(inputPhone)) {
                        ...
                        Cookies.set('userPhone', inputPhone, {expires: 7, path: '/baseFrame'});
                        // 验证通过刷新页面.
            ...
            // 2.用户已经登录,计时交互
            var startTime = parseInt(param['startTime']);
            var endTime = parseInt(param['endTime']);
            var seckillId = parseInt(param['seckillId']);
            // alert(startTime + "_" + endTime + "_" + seckillId);
            // 使用Ajax的GET请求从服务器端获取当前的系统时间.
            $.get(seckillJS.URL.now(), {}, function (result) {
                if(result && result['retCode']=='0') {
                    var nowTime = result['retObj'];
                    // 判断时间,计时交互
    ...
    // 执行真正的秒杀逻辑.
    handlerSeckill : function(seckillId, node,info) {
        // 获取秒杀地址,控制显示器执行秒杀
                ...
                if(exposer['exposed']) {    // 当开启秒杀的时候,获取md5数据进行处理,获取秒杀的地址
                    ...
                    $('#killBtn').one('click', function() {
                        ...
                        $.post(killUrl, {}, function(result) {
                        	if(result){
                        		var killResult = result['retObj'];
                                var state = killResult['state'];
                                var stateInfo = killResult['stateInfo'];
                                if(result['retCode']=='0') {
	                                var phone=killResult['successKilled']['userphone']
	                                var name=killResult['successKilled']['seckill']['name']
	                                // 显示秒杀的结果
	                                node.html('<label class="label label-success"> 手机号:' + phone + ' , 商品名称:' + name + ' , ' + stateInfo + '</label>');
	                                
                                }else if(result['retCode']=='1'){
                                	node.html('<label class="label label-success">'  + stateInfo + '</label>');
                                }
                                seckillJS.count(20,info);
                        	}
                        });
                    });
                    node.show();   // 显示被隐藏的节点
    ...
    count : function(wait,info){
    	if(wait==0){
    		parent.location.reload();   
    	}else{
    		info.html('<label class="label label-info">' + wait + "秒后关闭</label>");
    		wait--;
    		setTimeout(function(){
    			seckillJS.count(wait,info);
    		},1000)
    	}
    },
    ...
    countDown : function(seckillId, nowTime, startTime, endTime) {
        ...
        if(nowTime > endTime) {
            ...
            seckillBox.countdown(killTime, function(event) {
                ...
                seckillJS.handlerSeckill(seckillId, seckillBox, seckillInfo);
            });
        } else {
            // 秒杀开始
            seckillJS.handlerSeckill(seckillId, seckillBox, seckillInfo);
